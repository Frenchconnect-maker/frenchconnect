...
async function initCheckout(){
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // Vérifie confirmation email (si Auth l’exige)
      try {
        const uRes = await sb.auth.getUser();
        const u = uRes?.data?.user;
        if(u && u.email_confirmed_at === null){
          throw new Error("Email non confirmé. Ouvre le mail de confirmation Supabase, clique le lien, puis reconnecte-toi.");
        }
      } catch(eCheck){
      ...
      console.error(err);
      const rawMsg = (err?.message || "Erreur inconnue");
      const lower = rawMsg.toLowerCase();
      const nice = (lower.includes("email not confirmed") || lower.includes("not confirmed"))
        ? "Email non confirmé. Va dans ta boîte mail, ouvre le mail Supabase et clique sur le lien de confirmation, puis reconnecte-toi."
        : rawMsg;
      showMsg("err", nice);
      inFlight = false;
      btn.disabled = false;
