...
async function startStripeCheckout(sb, orderId){
  // On garde le check de session: tes tables (orders, addresses, etc.) sont liées à un user
  const { data: sess } = await sb.auth.getSession();
  const accessToken = sess?.session?.access_token;
  if(!accessToken) throw new Error("Session introuvable. Connecte-toi puis réessaie.");

  // ⚠️ Appel de l’Edge Function déployée
  const res = await fetch("https://mnsqfagfdahvhlfopfah.functions.supabase.co/stripe-create-checkout", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      // ✅ Auth utilisateur (recommandé si la function vérifie l’utilisateur)
      "Authorization": `Bearer ${accessToken}`,
      // ✅ Clé projet (publishable) pour Supabase Gateway
      "apikey": SUPABASE_KEY
    },
